...
export default function OrdersListScreen() {
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={{ marginTop: 8, color: "#555" }}>Đang tải đơn hàng...</Text>
      </View>
    );
  }
  ...
  if (orders.length === 0) {
    return (
      <View style={styles.center}>
        <Text style={{ color: "#777" }}>Bạn chưa có đơn hàng nào</Text>
      </View>
    );
  }
  ...
  const renderOrder = ({ item }: { item: Order }) => (
    ...
    >
      <View style={styles.orderHeader}>
        ...
        <Text style={[styles.status, item.status === "Processing" ? styles.processing : styles.completed]}>
          {item.status}
        </Text>
      </View>
      ...
      {item.items.slice(0, 2).map((i) => (
        <View key={i.productId} style={styles.itemRow}>
          <Image source={{ uri: i.image || "/no-image.png" }} style={styles.image} />
      ...
      {item.items.length > 2 && (
        <Text style={styles.moreText}>+ {item.items.length - 2} sản phẩm khác</Text>
      )}
